...
function RepoDetails() {
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/donsalvy/${id}`)
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/donsalvy/${id}/branches`)
    .then((response) => (response.json()))
    .then((data) => {
      setBranch(data)
    })
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/donsalvy/${id}/deployments`)
    .then((response) => (response.json()))
    .then((data) => {
      setDeployment(data)
    })
    ...
    return (
        <div id="repodetail">
            <div className="repodetail-card">
                ...
                <p>Live site: {deployment.length === 0 ? `none` : <a href={`https://donsalvy.github.io/${details.name}`}>donsalvy.github.io/{details.name}</a>}</p>
